...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    if (!token) {
        if(logger) {
            logger.error('JwtVerifier Failed -1', { token });
        }
    ...
    this._secretOrKeyProvider(req, token, function(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            if(logger) {
                logger.error('JwtVerifier Failed 0', {
                    token, secretOrKey, _verifOpts: self._verifOpts, secretOrKeyError
                })
            }
            self.fail(secretOrKeyError)
        } else {
            // Verify the JWT
            JwtStrategy.JwtVerifier(token, secretOrKey, { ...self._verifOpts, logger }, function(jwt_err, payload) {
                if (jwt_err) {
                    if(logger) {
                        logger.error('JwtVerifier Failed 1', {
                            token, secretOrKey, _verifOpts: self._verifOpts, jwt_err, payload
                        })
                    }
                    ...
                    var verified = function(err, user, info) {
                        if(err) {
                            if(logger) {
                                logger.error('JwtVerifier Failed 2', {
                                    token, secretOrKey, _verifOpts: self._verifOpts, jwt_err, payload
                                })
                            }
                            ...
                            if(logger) {
                                logger.error('JwtVerifier Failed 3', {
                                    token, secretOrKey, _verifOpts: self._verifOpts, jwt_err, payload
                                })
                            }
                    ...
                    try {
                        ...
                        if(logger) {
                            logger.error('JwtVerifier Failed 4', {
                                token, secretOrKey, _verifOpts: self._verifOpts, jwt_err, payload, ex
                            })
                        }
                        self.error(ex);
                    }
